...
#'  @normalize.int logical; whether interactions should be normalized
#' @cv.refit cv that is aware of refitting
slc_slc_int_all_splits <- function(X, y, method = "classif", output = "class", nbsplit = 10,
                                   normalize.int = TRUE, cv.refit = F, ii = "i1se"){
  ...
  for (r in seq(nsplit)) {
    ...
    if(cv.refit){
      ...
      fit[[r]]$refit <- FALSE
      cvfit[[r]] <- cv_sparse_log_contrast(fit[[r]], Z = ztr, y = ytr)
    }
    ...
    if(nnz[[r]][cvfit[[r]]$cv[[ii]]] != 0){
      refit_lc <- refit_selected_model(fit[[r]], i_selected = cvfit[[r]]$cv[[ii]],
                                                      Z = ztr, y = ytr)
    ...
    if(cv.refit){
      fit_int[[r]]$refit <- TRUE
      cvfit_int[[r]] <- cv_sparse_log_contrast_refit(fit_int[[r]],
                                                     Z = ztr, y = ytr,
                                                     additional_covariates =
                                                       xtr_int_normalization,
                                                     folds = cvfit[[r]]$folds)
      ...
      fit_int[[r]]$refit <- FALSE


      cvfit_int[[r]] <- cv_sparse_log_contrast(fit_int[[r]], Z = ztr, y = ytr,
                                               additional_covariates =
                                                 xtr_int_normalization,
                                               folds = cvfit[[r]]$folds)
    }
    ...
    print(nnz_int[[r]][cvfit_int[[r]]$cv[[ii]]])
    if(nnz_int[[r]][cvfit_int[[r]]$cv[[ii]]] > 1){#!= 0){

      refit_lc_int <- refit_selected_model(fit_int[[r]], i_selected = cvfit_int[[r]]$cv[[ii]],
      ...
      ## no refitting
      beta_int_est_norefit[, r] <- fit_int[[r]]$beta[, cvfit_int[[r]]$cv[[ii]]]
    }
